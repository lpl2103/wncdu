...
fn main() {
    println!("cargo:rerun-if-changed=assets/wncdu.rc");
    println!("cargo:rerun-if-changed=assets/wncdu.ico");

    let target_os = std::env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
    if target_os == "windows" {
        let out_dir = std::env::var("OUT_DIR").unwrap_or_default();
        let res_file = std::path::Path::new(&out_dir).join("wncdu.res");
        ...
        for cmd in &windres_candidates {
            let success = std::process::Command::new(cmd)
                .args([
                    ...
                    "coff",
                    "-o",
                    res_file.to_str().unwrap_or("wncdu.res"),
                ])
                .status()
        ...
        if !compiled && std::path::Path::new("assets/wncdu.res").exists() {
            let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_default();
            let precompiled = std::path::Path::new(&manifest_dir).join("assets/wncdu.res");
